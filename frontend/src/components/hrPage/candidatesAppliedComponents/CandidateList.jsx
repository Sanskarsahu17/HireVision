  ...
  return (
    <div className="mt-4">
      {candidates.map((candidate) => (
        ...
        >
          <h2 className="text-xl font-semibold text-white">{candidate.company}</h2>
          <p className="text-slate-400">Email: {candidate.email}</p>
          <p className="text-slate-400">Recruiter: {candidate.recruiterEmail}</p>
          ...
          <p className="text-slate-400">Uploaded: {new Date(candidate.uploadedAt).toLocaleDateString()}</p>
          <a href={candidate.resumePath} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">
            View Resume
          </a>
        </motion.div>
      ))}
